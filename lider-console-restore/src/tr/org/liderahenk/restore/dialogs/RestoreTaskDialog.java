package tr.org.liderahenk.restore.dialogs;
...
import tr.org.liderahenk.restore.constants.RestoreConstants;
import tr.org.liderahenk.restore.i18n.Messages;
import tr.org.liderahenk.restore.model.BackupServerConf;
...
public class RestoreTaskDialog extends DefaultTaskDialog {
	...
	public Control createTaskDialogArea(Composite parent) {
		...
		Label lblRestoreToPath = new Label(composite, SWT.NONE);
		lblRestoreToPath.setLayoutData(new GridData(SWT.RIGHT, SWT.CENTER, false, false, 1, 1));
		lblRestoreToPath.setText(Messages.getString("restore_path")); //$NON-NLS-1$
		
		textRestorePath = new Text(composite, SWT.BORDER);
		textRestorePath.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false, 1, 1));
		return null;
	}
	...
	private void createRestoreTableArea(Composite parent) {
		...
		try {
			selectedConfig = getRestoreServerConfig();
	...
	private String getInitialPath(String path) {
		int index = path.lastIndexOf(RestoreConstants.IP_ADDRESS_EXPRESSION);
		if (index > -1) {
	...
	private void createTableButtonArea(Composite parent) {
		...
		btnUpdateBackupServerConf.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				RestoreServerConfDialog dialog = new RestoreServerConfDialog(Display.getDefault().getActiveShell());
				dialog.create();
				dialog.open();
	...
	public Map<String, Object> getParameterMap() {
		...
		profileData.put(RestoreConstants.PARAMETERS.USERNAME, selectedConfig.getUsername());
		profileData.put(RestoreConstants.PARAMETERS.PASSWORD, selectedConfig.getPassword());
		profileData.put(RestoreConstants.PARAMETERS.DEST_HOST, selectedConfig.getDestHost());
		profileData.put(RestoreConstants.PARAMETERS.DEST_PORT, selectedConfig.getDestPort());
		
		
		if(currentPath.contains("."))
		
		profileData.put(RestoreConstants.PARAMETERS.SOURCE_PATH, currentPath);
		
		else
			
			profileData.put(RestoreConstants.PARAMETERS.SOURCE_PATH, currentPath.endsWith("/") ? currentPath : currentPath + "/");
		
		
		profileData.put(RestoreConstants.PARAMETERS.DEST_PATH, getDestPathFrom(currentPath));
		
		profileData.put(RestoreConstants.PARAMETERS.RESTORE_PATH, textRestorePath.getText() );
	...
	public String getPluginName() {
		return RestoreConstants.PLUGIN_NAME;
	}

	@Override
	public String getPluginVersion() {
		return RestoreConstants.PLUGIN_VERSION;
	}

	private BackupServerConf getRestoreServerConfig() throws Exception {
		IResponse response = null;
		try {
			response = TaskRestUtils.execute(RestoreConstants.PLUGIN_NAME, RestoreConstants.PLUGIN_VERSION,
					"GET_RESTORE_SERVER_CONFIG", false);
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			Notifier.error(null, Messages.getString("ERROR_ON_LIST"));
		}
		return (BackupServerConf) ((response != null && response.getResultMap() != null
				&& response.getResultMap().get("RESTORE_SERVER_CONFIG") != null)
						? new ObjectMapper().readValue(response.getResultMap().get("RESTORE_SERVER_CONFIG").toString(),
	...
	private String[] getBackupServerDirectories(String path) throws Exception {
		...
		try {
			...
			TaskRequest task = new TaskRequest(null, DNType.AHENK, RestoreConstants.PLUGIN_NAME,
					RestoreConstants.PLUGIN_VERSION, "LIST_RESTORE_SERVER_DIR", parameterMap, null, null, new Date());
